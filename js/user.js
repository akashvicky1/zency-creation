...
db.collection("products").orderBy("created", "desc").onSnapshot((snapshot) => {
  ...
  snapshot.forEach((doc) => {
    ...
    const discount = data.discount ? `<span style="text-decoration:line-through; color:#ccc">₹${data.price}</span> <span style="color:#00ffd5">₹${data.discount}</span>` : `<span style="color:#00ffd5">₹${data.price}</span>`;
    
    const imagesHTML = (data.images || []).map(url => `<img src="${url}" loading="lazy" />`).join("");
    const videoHTML = data.video ? `<video controls><source src="${data.video}" type="video/mp4"></video>` : "";
